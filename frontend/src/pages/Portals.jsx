    ...
    <PageShell active="portals">
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-[#0f5c5a]">
...
function PortalCard({ icon: Icon, title, description, points, loginPath, registerPath }) {
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#0f5c5a] text-white">
      ...
      <ul className="mt-5 flex-1 space-y-2.5">
        {points.map((point) => (
          <li key={point} className="rounded-xl bg-slate-50 px-3.5 py-2.5 text-sm font-medium text-slate-700">
            {point}
          </li>
        ))}
      </ul>
