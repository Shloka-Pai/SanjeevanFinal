...
function SiteNav({ active }) {
  const linkClass = (key) =>
    `rounded-full px-5 py-2 text-[15px] font-bold transition ${
      active === key
    ...
    <header className="relative z-10 border-b border-slate-200/80 bg-white/80 backdrop-blur">
      <div className="mx-auto flex max-w-[1500px] items-center justify-between px-8 py-5">
        <Link to="/" className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#0f5c5a] text-white">
            <HeartPulse className="h-6 w-6" />
          </div>
          <div>
            <p className="text-[23px] font-black tracking-tight text-slate-900">Sanjeevan</p>
            <p className="text-[13px] font-semibold text-slate-500">Hospital navigation network</p>
          </div>
        </Link>
...
function PageShell({ active, children }) {
  return (
    <div className="min-h-screen bg-[#f4f7f9] text-slate-900">
      ...
      <main className="mx-auto max-w-[1500px] px-8 py-12">{children}</main>
    </div>
  )
}
...
export default function Home() {
  return (
    <PageShell active="about">
      <section className="rounded-3xl border border-slate-200 bg-white px-8 py-10 shadow-sm md:px-14 md:py-14">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-teal-700">About</p>
        <h1 className="mt-3 max-w-4xl text-5xl font-black leading-tight tracking-tight text-slate-900 md:text-[56px]">
      ...
      <section className="mt-10">
        ...
        <div className="info-card-grid grid gap-5 md:grid-cols-3">
          {[
            {
              icon: Clock3,
              title: 'The problem',
            ...
            <article key={item.title} className="about-card flex h-full flex-col rounded-3xl p-6">
              <div className="about-card-icon mb-5 flex h-11 w-11 items-center justify-center rounded-xl">
      ...
      <section className="mt-12">
        ...
        <div className="info-card-grid grid gap-5 md:grid-cols-3">
          {[
            ...
            <article key={item.title} className="about-card flex h-full flex-col rounded-3xl p-6">
              <div className="about-card-icon alt-icon mb-5 flex h-11 w-11 items-center justify-center rounded-xl">
                <item.icon className="h-5 w-5" />
              </div>
              <h2 className="text-lg font-bold text-slate-900">{item.title}</h2>
